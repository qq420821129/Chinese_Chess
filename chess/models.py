...
class UserInfo(models.Model):
    ...
    state = models.CharField(max_length=10, choices=(('0', 'wait'), ('1', 'outline'), ('2', 'waitready'), ('3', 'ready'),
                                                     ('4', 'playing')), default='outline')
...
class Room(models.Model):
    rnum = models.AutoField(primary_key=True)  # 房间号
    red = models.ForeignKey(User, max_length=16, related_name='red', on_delete=models.CASCADE, null=True)  # 红方
    blue = models.ForeignKey(User, max_length=16, related_name='blue', on_delete=models.CASCADE, null=True)  # 蓝方
    state = models.CharField(max_length=16, choices=(('0', 'empty'), ('1', 'wait'), ('2', 'full')), default='wait')  # 状态

    def __str__(self):
        return "%s-%s-%s-%s" % (self.rnum, self.red, self.blue, self.state)
