$(document).ready(function(){
    ...
    putDef();
    showC();
    rtotalTime = 20*60*1000;
    btotalTime = 20*60*1000;
    redTime = 2*60*1000;
    blackTime = 2*60*1000;
    ...
    socket.onmessage = function(event){
        ...
        if (result.type === 'chess') {
            ...
            if (type > 0) {
                nowWho = 0;
                clearTimeout(rtimeid);
                redTime = 2 * 60*1000;
                var rt = toTime(redTime);
                $('.inred .step_time b').html(rt);
                bTotalCount()
            } else {
                nowWho = 1;
                clearTimeout(btimeid);
                blackTime = 2 * 60*1000;
                var bt = toTime(blackTime);
                $('.inblack .step_time b').html(bt);
                rTotalCount()
            }
            if (past === now) {
                return
            ...
            $('.tstep b').html(parseInt($('.tstep b').html()) + 1);
            console.log(side, type, '从', past[0], past[1], '移到', now[0], now[1]);
            onChose(9 - past[0], 8 - past[1], type * S);
            onChose(9 - now[0], 8 - now[1], type * S)
        }else if(result.type === 'talk'){    // 接收聊天信息
            var msg = result.dic;
            ...
            s.send(JSON.stringify({'type':'request', 'subtype':type, 'msg':result}))
        }else if(result.type === 'banance'){
            runNow = false;
            onMove = true;
            nowWho = undefined;
            moveList = [];
    ...
    $('#autoword').change(function () {
        arr = ["大家好!", "不要走!", "快点呀!天都黑了!"];
        index = parseInt($('#autoword').val()) - 1;
        if(index<0 || index >= arr.length ){
            return
        }
        $('.send input').val(arr[index]);
        $('.send #send').click()
    });
    ...
    $('.function .draw').click(function () {
        ...
        waitmessage = true;
        s.send(JSON.stringify({'type':'request', 'by':'draw'}))
    });

    $('.function .lose').click(function () {
        if (waitmessage) return
        waitmessage = true;
        s.send(JSON.stringify({'type':'request', 'by': 'lose'}))
    });

    $('.function .wback').click(function () {
        if (waitmessage) return
        waitmessage = true;
        if ((S==1 && nowWho != 0)|| (S==-1 && nowWho != 1)){
            alert('只有当前下棋方才可以悔棋')
            return
        }
        s.send(JSON.stringify({'type':'request', 'by':'wback'}))
        onMove = true
    });

    // 发送聊天信息
    var disablemsg = false
    $('#send').click(function () {    // 发送聊天信息
        if(disablemsg){
            alert('请勿频繁发送信息')
            return
        }
        disablemsg = true
        var sendMsg = {"type":"message", "content":$('#msg').val()};
        s.send(JSON.stringify(sendMsg));
        var div = document.createElement('div');
        div.innerHTML = $('#msg').val();
        $('.msg').append(div)
        $('#msg').val('')
        setTimeout(()=>{
            disablemsg = false
...
// history : past now type chess side
function addCover(){
    if(stepHistory.length){
        step = stepHistory.getReverse(-1)
        div = document.createElement('div')
        div.style.border = '3px solid black'

        $("#CS" + step.past[0]+"-"+step.past[1]).css('border','3px solid black').addClass('move')
        $("#CS" + step.now[0]+"-"+step.now[1]).css('border','3px solid black').addClass('move')
    }
}
function removeCover(){
    if(stepHistory.length){
        step = stepHistory.getReverse(-1)
        $("#CS" + step.past[0]+"-"+step.past[1]).css('border', '3px solid rgba(0,0,0,0)').removeClass('move')
        $("#CS" + step.now[0]+"-"+step.now[1]).css('border', '3px solid rgba(0,0,0,0)').removeClass('move')
    }
    cleanSt()
}


function movebackWithHistory(){
    removeCover();
    var first = stepHistory.pop();
    var chess = first['chess'];
    var type = first['type'];
    var y = first.past[0];
    var x = first.past[1];
    var j = first.now[0];
    var i = first.now[1];
    moveBack(j, i, y, x, type, chess)
}

function moveBack(y, x, j, i, type, chess) {
    var cla="";
    var tex="";
...
function bTotalCount(){
    btimeid = setTimeout(bTotalCount, 1000);
    btotalTime -= 1000;
    console.log(btotalTime);
    $('.inblack .total_time b').html(toTime(btotalTime));
    if(blackTime == 0){
        autoMove();
        clearTimeout(rtimeid)
    }else{
        blackTime -= 1000;
        $('.inblack .step_time b').html(toTime(blackTime))
    }
}

function rTotalCount(){
    rtimeid = setTimeout(rTotalCount, 1000);
    rtotalTime -= 1000;
    console.log('111111111', $('.inred .total_time b').html(toTime(rtotalTime)));
    $('.inred .total_time b').html(toTime(rtotalTime));
    if(redTime == 0){
        autoMove();
        clearTimeout(rtimeid)
    }else{
        redTime -= 1000;
        $('.inred .step_time b').html(toTime(redTime))
    }
}
    ...
    if ((S==1 && nowWho == 0)|| (S==-1 && nowWho == 1)) {
        for (var i = 0; i < map.length; i++) {
            for (var j = 0; j < map[i].length; j++) {
                if (map[i][j] * S > 0) {
                    onChose(i, j);
                    tmap = WhereCan(i, j, map[i][j] * S);
                    if (tmap) {
                        onChose(tmap[0][0], tmap[0][1]);
                        return
                    }
                }
            }
...
function toTime(sec){
    var min = Math.floor(sec/60/1000);
    if (min<=9){
        if (min > 0) {
            min = '0' + min
        }
    }
    var se = Math.floor((sec/1000)%60);
    if (se <=9){
        se = '0' + se
    }
    return min + ':' + se
...
function waitOther(dict){
    ...
    $('.tstep b').html(parseInt($('.tstep b').html()) + 1);
    window.s.send(JSON.stringify(dict))
}
...
Array.prototype.getReverse = function(num){
    return this[num + this.length]
}
Array.prototype.max_length = 4
    ...
    if(OnChoseNow){
        ...
        for(var q=0;q<moveList.length;q++){
            if (moveList[q][0]==j&&moveList[q][1]==i){  //如果移动列表中有该点坐标
                ...
                $('.nostep b').html(parseInt($('.nostep b').html()) + 1)
                break
            }
        }
    }
...
function move(y,x,j,i,eat){
    ...
    var dic = {"past": [y, x],
                ...
                "side":S};
    removeCover()
    stepHistory.MyPush(dic);
    dic = {"type":"chess", "dic":dic};
    map[j][i]=map[y][x];    //移动到新位置
    map[y][x]=0;            //清空原位置
    $("#CS"+j+"-"+i).html(  //标签更新
            "<div class='C "+cla+"' style='transform:translate("+(x-i)*45+"px,"+(y-j)*45+"px);'>"+tex+"</div>"
    ...
    setTimeout(function(){  //换边
        ...
        if (!end){
            ...
            if ((nowWho == 0 && S == 1) || (nowWho == 1 && S == -1) ){
                s.send(JSON.stringify({'type':'banance','result':'lose'}))
            }else {
                waitOther(dic);
                onMove = false;
                runNow = false;
                s.send(JSON.stringify({'type':'banance', 'result':'win'}))
            }
        }
    },700);
}
    ...
    while(true){
        ...
        if(map[q][w]==0){   //如果该点是空点
            ...
            if(ce){     //如果是之前还没有隔子,则跳过不执行
                if(map[q][w]*ci*S<0){ //如果已经隔子则判断该棋子是否对方棋子,是则添加到可行路径
                    var t=[];
                    t[0]=q;
                    t[1]=w;
                    tmap.push(t);
...
function fastMove(tmap,c,y,x){//c:0红 1黑
    ...
    if(map[y][x]==0){   //如果是空点
        ...
        if(map[y][x]*ci*S<0){ //如果是对方棋子,添加到可行路径中
            var t=[];
            t[0]=y;
            t[1]=x;
            tmap.push(t);
...
function maMove(tmap,c,y,x){
    ...
    function fastMa(tmap,y,x,ys,xs,c){
        ...
        if(map[y+ys][x+xs]==0){ //如果在馬的某个方向隔壁位置有棋子,则什么都不做,蹩脚
            ...
            if(map[y+ys+ys-yz][x+xs+xs-xz]*c*S<=0){   //如果该点有对方棋子或者为空点
                var t=[];
                t[0]=y+ys+ys-yz;
                t[1]=x+xs+xs-xz;
    ...
    function fastXiang(tmap,y,x,yy,xx,c,cy){
        if(y+yy*2<map.length&&y+yy*2>=0&&x+xx*2<map.length&&x+xx*2>=0){//棋子不超出地图位置
            ...
            if(map[y+yy][x+xx]==0){ //如果移动方向的中间为空点,即不蹩脚才继续,否则不予计算
                if(map[y+yy*2][x+xx*2]*c*S<=0){   //如果落子地方为空或者对方棋子才添加路径
                    var t=[];
                    t[0]=y+yy*2;
                    t[1]=x+xx*2;
                    tmap.push(t);
    ...
    function fastShi(tmap,y,x,yy,xx,c,cc){
        ...
        if(cc(y+yy)){   //如果竖直方向不超过上下三行
            if(x+xx>=3&&x+xx<=5){   //如果横向在第四行和第六行之间
                if(map[y+yy][x+xx]*c*S<=0){   //如果目标点是空点或者有地方棋子
                    var t=[];
                    t[0]=y+yy;
                    t[1]=x+xx;
                    tmap.push(t);   //保存到数组中
